...
namespace back_end.Controllers
{
    ...
    public class CuentasController : ControllerBase
    {
        ...
        [HttpPost("HacerCliente")]
        public async Task<ActionResult> HacerClient([FromBody] string usuarioId)
        {
            var usuario = await userManager.FindByIdAsync(usuarioId);
            await userManager.AddClaimAsync(usuario, new Claim("role", "cliente"));
            return NoContent();
        }

        [HttpPost("RemoverCliente")]
        public async Task<ActionResult> RemoverClient([FromBody] string usuarioId)
        {
            var usuario = await userManager.FindByIdAsync(usuarioId);
            await userManager.RemoveClaimAsync(usuario, new Claim("role", "cliente"));
            return NoContent();
        }

        [HttpPost("HacerProv")]
        public async Task<ActionResult> HacerProv([FromBody] string usuarioId)
        {
            var usuario = await userManager.FindByIdAsync(usuarioId);
            await userManager.AddClaimAsync(usuario, new Claim("role", "proveedor"));
            return NoContent();
        }

        [HttpPost("RemoverProv")]
        public async Task<ActionResult> RemoverProv([FromBody] string usuarioId)
        {
            var usuario = await userManager.FindByIdAsync(usuarioId);
            await userManager.RemoveClaimAsync(usuario, new Claim("role", "proveedor"));
            return NoContent();
        }
